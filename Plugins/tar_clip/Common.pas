...
Type TBiniFile=class(TIniFile) // объект, умеющий хранить двоичные данные в "теневом"
     ...
     TClip=class
      ...
      public
       ...
       Procedure SetClipboard(Handle:cardinal);
...
procedure PureClip; // удаляет из буфера информацию о форматировании, переводит Unicode
 // представление в обычный текст
 ...
 if OpenClipboard(Handle) then begin
  ...
  if Data<>0 then begin // существуют текст в юникоде
   ...
   if ValidText(DataPtrW,GlobalSize(Data) div 2) then begin
    ...
    GetMem(FData,NewSize+1);
    MoveMemory(FData,DataPtr,NewSize); // копируем на новое место
   end else begin
    ...
    GetMem(FData,NewSize+1);
    if WideCharToMultiByte(0,WC_COMPOSITECHECK,DataPtrW,-1,FData,NewSize,nil,nil)=0 then raiselastoserror;
     // перекодировали
   end;
   ...
   DataPtr:=GlobalLock(Data);
   GetMem(FData,NewSize+1);
   MoveMemory(FData,DataPtr,NewSize); // скопировали на новое место
  end;
  FData[NewSize]:=#0;
  GlobalUnlock(Data);
  CloseClipboard;
...
procedure TClip.SetClipboard(Handle: cardinal);
var Data:cardinal;
    P:PChar;
    I:integer;
begin
 if OpenClipboard(Handle) then begin
  Data:=GlobalAlloc(GHND or GMEM_DDESHARE,lstrlen(FData));
  P:=GlobalLock(Data);
  MoveMemory(P,FData,lstrlen(FData));
  GlobalUnlock(Data);
  EmptyClipboard;
  SetClipboardData(CF_TEXT,Data);
  I:=$419;
  Data:=GlobalAlloc(GHND or GMEM_DDESHARE,4); // простановка Locale продиктована личными соображениями и необязательна
  P:=GlobalLock(Data);
  MoveMemory(P,@I,4);
  GlobalUnlock(Data);
...
procedure PureClip;
 // удаляет из буфера информацию о форматировании, переводит Unicode представление
 // в обычный текст
var Clip:TClip;
begin
 Clip:=TClip.Create(GetForegroundWindow);
 if not Clip.Empty then begin
  Clip.SetClipboard(GetForegroundWindow);
