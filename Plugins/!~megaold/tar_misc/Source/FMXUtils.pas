...
procedure MoveFile(const SrcName, DestName: string);
...
begin
  Destination := ExpandFileName(DestName); { expand the destination path }
  ForceDirectories(Extractfiledir(Destination));
  if not RenameFile(SrcName, Destination) then { try just renaming }
  begin
    if HasAttr(SrcName, faReadOnly) then  { if it's read-only... }
      raise EFCantMove.Create(Format(SFCantMove, [SrcName])); { we wouldn't be able to delete it }
    CopyFile(SrcName, Destination); { copy it over to destination...}
    if not SysUtils.DeleteFile(SrcName) then
      raise EFCantMove.Create(Format(SFCantMove, [SrcName])); { ...and delete the original }
  end;
end;
